...
public class Deferred<T> {
	...
	private ArrayList<Runnable> dependOnMeList;

	public Deferred() {
		result = null;
		dependOnMeList = new ArrayList<Runnable>();
	}
	...
	public synchronized T get() {
		if (result == null)
			throw new IllegalStateException("Object hasn't been resolved yet");
		return result;
	...
	public synchronized void resolve(T value) {
		if (result != null) {
			throw new IllegalStateException("Object has been resolved already");

		}
		result = value;
		for (int i = 0; i < dependOnMeList.size(); i++) {
			dependOnMeList.get(i).run();
		}
		dependOnMeList.clear();
	...
	public synchronized void whenResolved(Runnable callback) {
		...
		if (!(dependOnMeList.contains(callback))) {
			if (!(isResolved()))
				dependOnMeList.add(callback);
			else
				callback.run();
		}
